...
# --- PWM KONFIGURATION (Server Safe) ---
# Funktioniert auf Raspberry Pi, ignoriert Fehler auf Servern/PCs
...
def get_history_filename(sensor_id):
    # Dateiname basiert auf Sensor ID.
...
def save_history(sensor_id, data_list):
    filename = get_history_filename(sensor_id)
    now = time.time()
    # Nur Daten der letzten 24h behalten
...
def get_current_data(api_key, sensor_id, lat, lon):
    try:
        ...
        # B) Luftdaten
        # Strategie: Erst OWM holen, dann ggf. mit Sensor überschreiben
        ...
        # Falls Sensor-ID da ist -> Überschreiben
        if sensor_id:
        ...
        data = {
            ...
            "pm10": val_pm10,
            "nox": val_nox,
            "location": w_res.get("name", "Unbekannt")
        }
        return data
...
def index():
    ...
    # Nur laden, wenn die Pflicht-Parameter (Key + Koordinaten) da sind
    ...
    date_day = f"{days[now.weekday()]}, {now.strftime('%d.%m.%Y')}"
